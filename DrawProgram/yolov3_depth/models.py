...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        p = p.view(bs, self.na, self.nc + 5 + 1, self.ny, self.nx).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        ...
        if self.training:
            ...
            io[..., 0:2] = torch.sigmoid(io[..., 0:2]) + self.grid_xy  # 01 for xy
            io[..., 2:4] = torch.exp(io[..., 2:4]) * self.anchor_wh  # wh yolo method
            # io[..., 2:4] = ((torch.sigmoid(io[..., 2:4]) * 2) ** 3) * self.anchor_wh  # wh power method
            io[..., :4] *= self.stride

            #bys 这里depth输出也过sigmoid到0~1之间
            ...
            return io.view(bs, -1, self.nc + 5 + 1), p
