...
def compute_loss(pred, targets, model):  # predictions, targets, model
    ...
    lcls, lbox, lobj ,ldepth = ft([0]), ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec, tdepth = build_targets(model, targets)
    ...
    for i, pred_i in enumerate(pred):  # layer index, layer predictions
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx 标签框，
        tobj = torch.zeros_like(pred_i[..., 0])  # target obj

        # Compute losses
        nb = len(b)
        if nb:  # number of targets
            ps = pred_i[b, a, gj, gi]  # prediction subset corresponding to targets 0123对应xywh,4对应conf,5~-1对应class_prob
            ...
            # Depth Loss
            pdepth = torch.sigmoid(ps[:, 5])-0.5
            pdepth *= 2
            #print("\nPdepth",pdepth)
            #print("\nTdepth",tdepth[i])
            ldepth += torch.mean(torch.abs(pdepth-tdepth[i]))

            if model.nc > 1:  # cls loss (only if multiple classes)
                t = torch.zeros_like(ps[:, 6:])  # targets
                t[range(nb), tcls[i]] = 1.0 #将这个box对应的目标类别置为1
                lcls += BCEcls(ps[:, 6:], t)  # BCE

        lobj += BCEobj(pred_i[..., 4], tobj)  # obj loss 是否含有目标的conf损失
    ...
    return loss, torch.cat((lbox, lobj, lcls, loss, ldepth)).detach()

#bys
#从标签建立用于训练的标签矩阵
def build_targets(model, targets):
    # targets = [image, class, x, y, w, h]
    # targets = [image, class, x, y, w, h, depth]
    # number of targets
    nt = len(targets)
    #print("Target")
    #print(targets.shape)
    #print(targets)
    tcls, tbox, indices, av, tdepth = [], [], [], [], []
    ...
    return tcls, tbox, indices, av, tdepth
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5):
    ...
    for image_i, pred in enumerate(prediction):
        # Multiply conf by class conf to get combined confidence #bys 0123xywh 4conf 5depth 6+class
        ...
        # Detections ordered as (x1y1x2y2, obj_conf, class_conf,depth#bys ,class_pred )
        pred = torch.cat((pred[:, :5], class_conf.unsqueeze(1), pred[:, 5:6], class_pred), 1)
        ...
        for c in pred[:, -1].unique():
            ...
            if n == 1:
                ...
                dc = dc[:1]  # limit to first 100 boxes: https://github.com/ultralytics/yolov3/issues/117
        ...
        '''
        output[image_i] = torch.unsqueeze(pred[0], 0)
